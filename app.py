...
st.title("📦 Cebu Routing Optimizer with Smart Address Suggestions")

ors_key = st.text_input("🔑 Enter OpenRouteService API Key", type="password")
uploaded_file = st.file_uploader("📁 Upload Delivery Excel File", type=["xlsx"])
...
if uploaded_file and ors_key:
    ...
    if len(valid_df) >= num_trucks:
        ...
        Marker(depot_coords, tooltip="🏭 Depot").add_to(m)
